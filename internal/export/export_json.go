...
// ExportJSON writes the entire database state as a single JSON file.
// If namespace is non-empty, only observations (and their related data) for
// that namespace are included. Returns the observation count.
func ExportJSON(ctx context.Context, db *sql.DB, namespace, outputPath string) (int, error) {
	export := FullExport{
		Version:    "1.0",
	...
	// Load all tables before any long-lived cursor (SQLite MaxOpenConns=1 gotcha).
	var err error
	...
	if err != nil {
		return 0, fmt.Errorf("load observation_links: %w", err)
	}

	export.FileObservations, err = loadFileObservationsJSON(ctx, db)
	if err != nil {
		return 0, fmt.Errorf("load file_observations: %w", err)
	}
	...
	if err != nil {
		return 0, fmt.Errorf("load temporal_mentions: %w", err)
	}

	export.Sessions, err = loadSessionsJSON(ctx, db, namespace)
	if err != nil {
		return 0, fmt.Errorf("load sessions: %w", err)
	}

	export.Reflections, err = loadReflectionsJSON(ctx, db, namespace)
	if err != nil {
		return 0, fmt.Errorf("load reflections: %w", err)
	}

	export.ConsolidationRuns, err = loadConsolidationRunsJSON(ctx, db)
	if err != nil {
		return 0, fmt.Errorf("load consolidation_runs: %w", err)
	}
	...
	return len(export.Observations), nil
}
